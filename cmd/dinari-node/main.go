...
// 🔥 COMPLETE initializeNode function
...
func (n *Node) Shutdown(ctx context.Context) error {
	...
	if n.metricsServer != nil {
		...
		if err := n.metricsServer.Shutdown(metricsCtx); err != nil {
	...
	if n.pprofServer != nil {
		pprofCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
		defer cancel()

		if err := n.pprofServer.Shutdown(pprofCtx); err != nil {
...
func (n *Node) commitFinalState(ctx context.Context) error {
	...
	if err := n.stateDB.CommitState(height, fmt.Sprintf("%x", block.Header.Hash)); err != nil {
		return fmt.Errorf("state commit failed: %w", err)
	}

	n.logger.Info("Final state committed",
		zap.Uint64("height", height),
		zap.String("hash", fmt.Sprintf("%x", block.Header.Hash[:8])),
	)
	return nil
}
...
func (n *Node) gracefulDatabaseClose(ctx context.Context) error {
	n.logger.Info("Closing database with final cleanup...")
...
func (n *Node) createShutdownMarker() error {
	...
	return os.WriteFile(markerPath, []byte(markerData), 0600)
}
